...
console.log("Canvas initialized:", {
    ...
    isLargeMonitor: window.innerWidth >= 1920,
    optimizedFor32inch: window.innerWidth >= 1920 && window.innerWidth <= 2560
});
...
// 📢 Получаем коэффициент ЗАРАНЕЕ (ДО начала flying!)
socket.on('next_coefficient', function (data) {
    console.log('📢 NEXT COEFFICIENT received (before flying!):', data);
    var $obj = typeof data == "string" ? JSON.parse(data) : data;
    if ($obj && $obj.cf) {
        // Сохраняем следующий коэффициент
        $game.next_cf = parseFloat($obj.cf);
        $game.win_cf = parseFloat($obj.cf);
        console.log('✅ Next round coefficient set:', $game.win_cf);
    }
});
...
$(document).ready(function () {
    ...
    console.log("📺 Large monitor (>=1920px, 32\"):", window.innerWidth >= 1920);
    console.log("🎯 Optimizations active:", window.innerWidth >= 1920);
